<?php include('config.php');
	...
	if ((empty($_POST['idreserva']))) {
		...
		$consulta= "SELECT * FROM reserva WHERE Id_Reserva='$idreserva' and Visible=1 and Estado='confirmada'";
		$consulta_execute = $conexion->query($consulta);
		if($consulta_execute->num_rows){
			$sql= "UPDATE `reserva` SET `Estado`='cancelada', `Canc_Huesped`=1  WHERE `reserva`.`Id_Reserva` = '$idreserva'";
