...
<?php
	...
	{	
		...
		//Consultas SQL
		$consulta = "SELECT * FROM usuario WHERE Visible=1 and Id_TipoDeUsuario=2 ORDER BY Id_Usuario ASC";
		$consulta_execute = $conexion->query($consulta);
		$total_resultados=$consulta_execute->num_rows;
		$total_paginas=ceil($total_resultados/$TAMANO_PAGINA);
		$consulta = "SELECT u.Id_Usuario, u.Nombre, u.Apellido, u.Email, u.FechaNac, u.Telefono, u.Premium, u.FechaAlta, t.Nombre AS NombreTipo FROM usuario u inner JOIN tipodeusuario t ON u.Id_TipoDeUsuario = t.Id_Tipo WHERE u.Visible=1 and Id_TipoDeUsuario=2 ORDER BY FechaAlta ASC LIMIT ".$inicio.",".$TAMANO_PAGINA."";
		$consulta_execute = $conexion->query($consulta);
?>
<html>
	<head>
		<meta charset="utf-8">
		<title>CouchInn - Listar Administradores</title>
			...
			<nav>
				<div class="nav-wrapper white z-depth-3">
					...
					<ul class="right hide-on-med-and-down">
						...
						<li><a href="../ayuda.php#listadmin" class="light-green"><i class="large material-icons">help_outline</i></a></li>
					</ul>
				...
				<div class="section">
					...
					<div class="row">
						<table class="col s12 highlight responsive-table">
							...
							echo '<tr>
									<td class="center">No existen usuarios Administradores.</td>
								</tr>';
							}
						?>
						</table>
					...
					<ul class="pagination center">
						<?php
							if ($pagina==1){
								...
								$paginaant=$pagina-1;
								echo '<li class="waves-effect"><a href="listaradmin.php?pagina='.$paginaant.'"><i class="material-icons">chevron_left</i></a></li>';
							}
							if ($total_paginas > 1){
								for ($i=1;$i<=$total_paginas;$i++){ 
									if ($pagina == $i){
										...
										echo '<li class="waves-effect"><a href="listaradmin.php?pagina='.$i.'">'.$i.'</a></li>';
									}
								}
								if ($pagina==$total_paginas){
									...
									$paginapos=$pagina+1;
									echo '<li class="waves-effect"><a href="listaradmin.php?pagina='.$paginapos.'"><i class="material-icons">chevron_right</i></a></li>';
								}
